...
<%--显示房型信息的模态框--%>
<div class="modal fade" id="roomTypeModal" tabindex="-1" role="dialog" aria-labelledby="myModalLabel"
     aria-hidden="true">
    <div class="modal-dialog">
        <div class="modal-content">
            <div class="modal-header">
                <button type="button" class="close" data-dismiss="modal" aria-hidden="true">&times;</button>
                <h4 class="modal-title" id="myModalLable">房型信息</h4>
            </div>
            <div class="modal-body">
                <form class="form-horizontal">
                    <div class="form-group">
                        ...
                        <div class="col-sm-10">
                            <p class="form-control-static" id="hotel_name_static"></p>
                        </div>
                    </div>
                    <div class="form-group">
                        <label class="col-sm-2 control-label">房型</label>
                        <div class="col-sm-10">
                            <p class="form-control-static" id="room_type_static"></p>
                        </div>
                    </div>
                </form>
            </div>
        ...
        <div class="col-md-9">
            <div class="col-md-12" style="background-color:rgba(80,77,77,0.9);height:100%;color:#FFFFFF">
                <div class="row" style="height:50px">
                    <div class="col-md-3" style="margin-top:10px;text-align:center;font-size:20px">酒店信息</div>
                </div>
                        ...
                        <div class="col-md-12">
                            <table class="table table-hover" id="hotel_table">
                                ...
                                <tr style="color: #ffffff">
                                    ...
                                    <th style="text-align:center"><img src="../pics/UIpic/tools.png"
...
<script type="text/javascript">
    ...
    function build_hotel_table(result) {
        ...
        var hotel = result.extend.pageInfo.list;
        $.each(hotel, function (index, item) {
            var hotelIdTd = $("<td></td>").append(item.hotelId);
            var hotelNameTd = $("<td></td>").append(item.hotelName);
            var starsTd = $("<td></td>").append(item.stars);
            var Btn = $("<button></button>").addClass("btn btn-primary btn-sm type_btn")
                .append($("<span></span>").addClass("glyphicon glyphicon-check")).append("查看房型");
            //为按钮添加一个自定义的属性，来表示当前酒店id
            Btn.attr("type-id", item.hotelId);
            var btnTd = $("<td></td>").append(Btn);
            //append方法执行完成以后还是返回原来的元素
            $("<tr></tr>").append(hotelIdTd)
                .append(hotelNameTd)
                .append(starsTd)
                .append(btnTd)
    ...
    $(document).on("click", ".type_btn", function () {
        var hotelId = $(this).attr("type-id");
        $("#roomTypeModal").modal({
            backdrop: "static"
        });
        $.ajax({
            url: "${APP_PATH}/hotel/" + hotelId,
            type: "GET",
            success: function (result) {
                var room_typesData = result.extend.room_types;
                $("#hotel_name_static").text(room_typesData[0].hotelName);
                $("#room_type_static").text(room_typesData[0].roomName + "　　　　" + room_typesData[1].roomName + "　　　　" + room_typesData[2].roomName);
            }
        });
    })
</script>
